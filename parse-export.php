...
function get_content($answer) {
    ...
    echo '<pre>';
    // print_r($array['children'][1]['children'][0]['children'][6]['children'][3]['children'][1]['children']);
    foreach ($array['children'][1]['children'][0]['children'][6]['children'][3]['children'][1]['children'] as $key => $value) {
        // print_r($value);
        // echo $value['children'][2]['children'][0]['href'];
        if ( date('Y-m-d', strtotime($value['children'][2]['html'])) == date('Y-m-d')) {
            $link_dowloand = 'https://client.atlanticexpresscorp.com/'.$value['children'][3]['children'][0]['href'];
            dowload($link_dowloand, rand(0,324234234));
            echo 'test ';
        } else {
